...
export const SymbolSelectSection = ({ onBack, disabled, onSelect }: Props): JSX.Element => {
  const [symbols, setSymbols] = useState<alpaca.Asset[]>([]);
  ...
  useEffect(() => {
    const results = symbols.filter((symbol) => symbol.name.toLowerCase().includes(searchTerm));
